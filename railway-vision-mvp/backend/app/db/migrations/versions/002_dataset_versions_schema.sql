...
CREATE TABLE IF NOT EXISTS dataset_versions (
  ...
  version VARCHAR(64) NOT NULL,
  asset_id VARCHAR(36) NOT NULL REFERENCES data_assets(id) ON DELETE CASCADE,
  asset_purpose VARCHAR(32) NOT NULL DEFAULT 'training',
  buyer_tenant_id VARCHAR(36) NULL REFERENCES tenants(id) ON DELETE SET NULL,
  source_type VARCHAR(32) NOT NULL DEFAULT 'result_export',
  summary JSONB NOT NULL DEFAULT '{}'::jsonb,
  created_by VARCHAR(36) NOT NULL REFERENCES users(id),
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT uq_dataset_key_version UNIQUE (dataset_key, version)
);
...
CREATE INDEX IF NOT EXISTS idx_dataset_versions_key ON dataset_versions(dataset_key, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_dataset_versions_buyer_tenant_id ON dataset_versions(buyer_tenant_id);
CREATE INDEX IF NOT EXISTS idx_dataset_versions_asset_id ON dataset_versions(asset_id);
