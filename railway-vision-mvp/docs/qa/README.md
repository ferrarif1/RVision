...
- 当前训练相关 QA 结论覆盖：作业对象、worker 接入、资产/模型受控拉取、候选模型回收入库、取消/重试/改派控制动作、审计痕迹。
- 当前运行时硬化门禁已覆盖：错误密码拒绝、空文件拒绝、非法类型拒绝、重复上传复用。
- 当前快速识别 QA 已覆盖：上传单图、输入对象提示词、自动选模、边缘检测、标注结果校验、轻量修订保存、结果打包为数据集资产、数据集版本推荐与对比、样本级差异统计、数据集版本样本预览与预览文件拉取。
