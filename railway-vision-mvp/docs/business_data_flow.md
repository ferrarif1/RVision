...
> 当前状态说明：本文中的训练链路包含目标态设计。当前 MVP 已经落地的是资产用途治理、模型提审发布、Pipeline 编排推理与审计回传；真正的训练作业控制面与远程训练 worker 尚未实现。
...
> 注：本节描述的是目标态训练治理流。当前仓库里已经支持训练/微调资产语义、模型提交与审批发布，但尚未支持真实训练作业调度。
...
- 当前实现提醒：平台对“训练/微调”的支持还停留在治理与交付语义，不应对外表述为已具备完整分布式训练平台。
