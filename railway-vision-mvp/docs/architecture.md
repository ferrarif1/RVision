...
- 当前已部分实装：
  - 训练作业控制面骨架
  - 远程训练 worker 注册、心跳、拉取作业、受控拉取训练资产/基线模型、上传候选模型、状态回传
- 当前未实装：
  - 真实训练执行引擎
  - 自动验证晋级与训练容量治理
- 因此本文中“训练环境 / 托管训练”相关内容应理解为“控制面最小骨架已具备，执行面仍是目标态补齐项”。
...
1. 供应商在平台上传 `model_package.zip` 到中心端（状态=`SUBMITTED`），并附带来源类型、基线模型、微调轮次、训练数据批次等元数据；当前 MVP 还新增了训练作业控制面与候选模型回收闭环，但仍未直接内置训练引擎。
...
1. 平台创建训练作业，绑定训练资产、验证资产、目标模型版本和基线模型。
2. 远程 training worker 通过专用凭证注册、心跳并拉取待执行作业。
3. worker 按作业授权拉取训练资产和基线模型包。
4. worker 完成训练后上传候选模型包，平台执行验签、哈希校验并自动入库为 `SUBMITTED`。
5. 平台后续再走管理员验证、审批和发布。
