...
### ~~T9. AI 会话改成“纯对话优先 + 会话内执行闭环”~~

- Priority: P0
- Status: Done
- Result:
  - ~~默认返回 chat；只有确有可执行动作时才返回结构化 direct action，不再硬塞 workflow 条~~
  - ~~`测试模型名称 / 最近训练怎么样 / 最近结果在哪 / 为什么最近任务失败 / 有没有待验证模型` 已改成纯对话查询~~
  - ~~会话内结果回灌训练闭环已补齐：结果确认 -> 整理训练数据 -> 创建训练作业 -> 刷新训练状态~~
  - ~~会话内模型治理闭环已补齐：定位待验证模型 -> 直接审批 -> 直接发布，并保留专家页兜底~~
  - ~~会话内查询动作已补齐：查看结果页 / 刷新训练状态 / 打开排障中心 / 打开模型中心~~
...
### D83. AI 会话主链进入“纯对话优先 + 会话内执行”形态

- Status: Done
- Result:
  - `测试模型名称 / 最近训练怎么样 / 最近结果在哪 / 有没有待验证模型`
  已支持直接纯对话查询，不再错误挂 workflow 条。
  - 新会话里 `chat-only` 消息不再被旧 `workflow draft / primary_action` 污染。
  - 识别链路已支持会话内继续执行：
    - 上传样例
    - 选择已发布模型
    - 发起识别
